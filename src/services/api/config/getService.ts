...
export const getService = async <InputType, ReturnType>({
  url,
  configs,
  customs = {
    isNormalize: true,
  ...
  if (customs?.isNormalize) {
    const response = await baseClient.get<ReturnType, CustomAxiosResponse<ReturnType>>(
      url,
      {
        ...configs,
        params: data,
  ...
  return await baseClient.get<ReturnType, CustomAxiosResponse<ReturnType>>(url, {
    ...configs,
    params: data,
  });
};
